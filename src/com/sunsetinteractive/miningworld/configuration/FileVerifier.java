...
public class FileVerifier {
	...
	public FileVerifier(MiningWorld miningWorld, FileManager fileManager, String configurationFileName,
			boolean applyComments) {
		this.fileManager = fileManager;

		loadedFiles = new EnumMap<>(File.Type.class);

		java.io.File configFile = new java.io.File(miningWorld.getDataFolder(), configurationFileName);
		...
		if (applyComments) {
			loadedFiles.put(File.Type.RESOURCE,
					new File(null, null, YamlConfiguration.loadConfiguration(new InputStreamReader(fileManager
							.getConfigContent(new InputStreamReader(miningWorld.getResource(configurationFileName)))))));
		} else {
			loadedFiles.put(File.Type.RESOURCE, new File(null, null, YamlConfiguration
					.loadConfiguration(new InputStreamReader(miningWorld.getResource(configurationFileName)))));
		}
	}
